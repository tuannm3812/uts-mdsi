---
type: subject-index
status: active
---

# CODE Subject Name

Semester:
Source folder:

## Folder Convention

- `lectures/`: weekly lecture summaries and extracted concepts
- `assignments/`: assessment briefs, rubric checklists, draft reviews
- `notebooks/`: notebook explanations, experiment logs, code review notes
- `sources/`: source-file inventories and reading notes tied to this subject
- `questions/`: exam drills, open questions, and tutor prompts

## What This Subject Is About

- Core question:
- Main skills:
- Important tools:
- Assessment shape:

## Weekly Map

| Week | Topic | Source Notes | Concepts | Questions |
|---|---|---|---|---|
| 1 |  |  |  |  |

## Key Concepts

- 

## Assessments

| Assessment | Due Date | Weight | Status | Notes |
|---|---:|---:|---|---|
|  |  |  |  |  |

## LLM Tutor Prompts

- Explain this subject's current topic at beginner, master student, and practitioner levels.
- Generate 10 exam-style questions from the notes in this page.
- Check my assignment draft against the marking rubric and list concrete gaps.
- Link this subject to other subjects in my master's wiki.

## Open Questions

- 
