...
| Barco et al. (2024) | Self-supervised temporal hotspot detection (Europe) | Motivates label-efficient temporal learning | Preprint status, Australian transfer, calibration |
...
| Phan et al. (2022) | Semantic explanations for real-time event-stream detection | Shows explainability already exists elsewhere in detection | Whether explanations are faithful, stable, and applicable to satellite false alarms |
...
**Transformers.** Rad et al. (2024) and Barco et al. (2024) both confirm that applying a time-series or vision transformer to fire detection is not by itself a novel contribution. The open question is what a transformer would add specifically to reliability rather than raw detection.
