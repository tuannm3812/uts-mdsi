---
type: subject-index
status: active
code: "36127"
name: "36127 Innovation Lab: Capstone Project"
---
# 36127 Innovation Lab: Capstone Project

Semester: [Sem4 2026 Spring](../../01-semesters/sem4-2026-spring.md)
Source folder: `/Users/tuannm3812/Library/CloudStorage/GoogleDrive-tuannm3812@gmail.com/My Drive/01_Study/0. Master/6. UTS Drive/Sem4_2026 Spring/36127 Innovation Lab Capstone Project`

## Folder Convention

- `lectures/`: capstone briefing materials, industry sponsor workshops, and project management guidance
- `assignments/`: project charters, client deliverables, technical documentation, and final presentations
- `notebooks/`: production data pipelines, model code repositories, and deployment prototypes
- `sources/`: client data inventory, stakeholder requirements, and technical documentation

## What This Subject Is About

- Executing a major end-to-end data science / AI capstone project for industry partners or real-world stakeholders.
- Data engineering, modeling, evaluation, deployment packaging, and stakeholder presentation.
- Translating complex technical solutions into actionable business value and strategic recommendations.

## Source Subfolders

- `capstone_charters`
- `client_briefs`
- `prototype_code`

## Key Concepts

- [Data Science Workflow](../../03-shared-concepts/data-science-workflow.md)
- [Machine Learning](../../03-shared-concepts/machine-learning.md)
- [Artificial Intelligence](../../03-shared-concepts/artificial-intelligence.md)

## Assessments

| Assessment | Due Date | Weight | Status | Notes |
|---|---:|---:|---|---|
| AT1 Capstone Project Charter |  |  | Draft |  |
| AT2 Mid-Project Prototype & Review |  |  | Draft |  |
| AT3 Final Solution & Executive Showcase |  |  | Draft |  |

## LLM Tutor Prompts

- Review our capstone architecture diagram and data pipeline design.
- Help refine executive summary and stakeholder value proposition for our final presentation.
- Critique model evaluation metrics against client business objectives.

## Curated Study Layer

- [Spring 2026 Capstone Topic Picker — 16 verified topics](sources/capstone-topic-picker-2026.md)
- [Spring 2026 Kickoff Requirements](sources/36127-kickoff-requirements-spring-2026.md)
- [Instructor Instructions Paste Area](assignments/instructor-instructions-capture.md)
- [Group Formation Outreach](assignments/group-formation-outreach-2026.md)
- [Assignment Dashboard](assignments/README.md)

## Import Summary

- Copied/readable files: 1 raw Capstone source document
- Referenced large or binary files: 0
- Source inventory: [sources/drive-source-inventory.md](sources/drive-source-inventory.md)
