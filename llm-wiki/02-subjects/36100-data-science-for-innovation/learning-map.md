---
type: learning-map
subject: 36100-data-science-for-innovation
code: 36100
semester: Sem1 2025 Autumn
status: active
---

# 36100 Data Science for Innovation - Learning Map

## Purpose

Use this page as the curated entry point for the subject. Raw copied files are useful evidence, but this page should become the LLM-friendly map of what matters.

## Core Focus

- Data science problem framing
- Innovation-oriented analysis
- Evidence-based recommendations
- Communication of insight

## Connected Concepts

- [Data Science Workflow](../../03-shared-concepts/data-science-workflow.md)
- [Research Methods](../../03-shared-concepts/research-methods.md)
- [Data Visualisation](../../03-shared-concepts/data-visualisation.md)

## Study Workflow

1. Review the weekly lecture material in [lectures](lectures/README.md).
2. Convert important source material into concise Markdown notes.
3. Link durable ideas to shared concepts.
4. Track assessment requirements in [assignments](assignments/README.md).
5. Use [questions](questions/README.md) for exam drills and unresolved questions.

## Imported Source Profile

| Bucket | Source Count |
|---|---:|
| Lectures | 14 |
| Assignments | 45 |
| Notebooks | 23 |
| Sources | 4 |

## Key Raw Files To Triage

### Lectures

- Doing Data Science.pdf
- [Joel_Grus]_Data_Science_from_Scratch_First_Princ.pdf
- AI_Ethics.pdf
- 03_uts_hypothesis_testing_and_evaluation.pdf
- 01_uts_introduction.pdf
- 05_uts_clustering_and_dimensionality_reduction.pdf
- 06_uts_linear_regression_and_logistic_regression.pdf
- 02_uts_data_exploration_with_python.pdf

### Assignments

- League of Legends Ranked Match Data  Season 15 (EUN).csv
- league_data.xlsx
- SlowbutSure_Assignment 2_Code.pdf
- ManhTuanNguyen_25739083 - Quiz 1 Question 7.pdf
- case_study2.xlsx
- synthetic_fraud_dataset.csv
- ManhTuanNguyen_10_unstructed_data.pdf
- ManhTuanNguyen_05_clustering_and_dimensionality_reduction.pdf

### Notebooks

- 05_clustering_and_dimensionality_reduction_solution.ipynb
- 05_clustering_and_dimensionality_reduction.ipynb
- 02_data_exploration_with_python3_solution.ipynb
- 02_data_exploration_with_python.ipynb
- 04_association rules.ipynb
- 04_association rules_solution.ipynb
- 06_regression.ipynb
- Groceries.csv

## Assessment Links

- [ASSIGNMENT 2](assignments/assignment-2.md)

## LLM Study Prompts

- Summarise the lecture files for this subject into weekly notes, key concepts, formulas, examples, and assessment relevance.
- Build a glossary for this subject and link each term to shared concepts where possible.
- Create a revision quiz from the learning map and the curated lecture notes.
- Identify which raw files are most useful for assessment preparation and why.

## Maintenance Checklist

- [ ] Weekly notes created
- [ ] Assignment pages created
- [ ] Key notebooks explained
- [ ] Shared concepts linked
- [ ] Exam/revision questions added
