---
type: learning-map
subject: 36103-statistical-thinking-for-data-science
code: 36103
semester: Sem2 2025 Spring
status: active
---

# 36103 Statistical Thinking for Data Science - Learning Map

## Purpose

Use this page as the curated entry point for the subject. Raw copied files are useful evidence, but this page should become the LLM-friendly map of what matters.

## Core Focus

- Statistical reasoning under uncertainty
- Sampling, inference, and assumptions
- Model interpretation
- Evidence strength and limitations

## Connected Concepts

- [Statistics](../../03-shared-concepts/statistics.md)
- [Research Methods](../../03-shared-concepts/research-methods.md)
- [Data Science Workflow](../../03-shared-concepts/data-science-workflow.md)

## Study Workflow

1. Review the weekly lecture material in [lectures](lectures/README.md).
2. Convert important source material into concise Markdown notes.
3. Link durable ideas to shared concepts.
4. Track assessment requirements in [assignments](assignments/README.md).
5. Use [questions](questions/README.md) for exam drills and unresolved questions.

## Imported Source Profile

| Bucket | Source Count |
|---|---:|
| Lectures | 16 |
| Assignments | 546 |
| Notebooks | 16 |
| Sources | 5 |

## Key Raw Files To Triage

### Lectures

- Lecture 4_UTSV3_A.pdf
- Lecture 4_UTSV3_A.pdf
- Lecture 6_Final-1.pdf
- Lecture 2_Finalv3 NonMath.pdf
- Lecture 6V2-3.pdf
- Lecture 2_Finalv2-2.pdf
- Lecture 2_Finalv2-2.pdf
- Lecture 6V2-1.pdf

### Assignments

- Copy of Accessible-content-practices-PPT-example-edited.pptx
- TeleCom_Data.csv
- 36103_sp25_assignment_2_ManhTuanNguyen_25739083_backup_1.ipynb
- 36103_sp25_assignment_2_ManhTuanNguyen_25739083_by_hour.ipynb
- 36103_sp25_assignment_2_ManhTuanNguyen_25739083_by_day_with_weather.ipynb
- 36103_sp25_assignment_1_ManhTuanNguyen_25739083.ipynb
- 36103_sp25_assignment_3_ManhTuanNguyen_25739083.ipynb
- ManhTuanNguyen_Assignment_1_20250903.pdf

### Notebooks

- w6_IMDB.csv
- 01_EDA_25v3.ipynb
- 04_Part2_CNN_2025_v2.ipynb
- 02_LinearRegression_25V2.ipynb
- 03_LogisticRegression-2025.ipynb
- w3_Tu1HousingPriceData-1.csv
- w6_Tu1HousingPriceData-3.csv
- w7_Tu1HousingPriceData-4.csv

## Assessment Links

- [AT2](assignments/at2.md)

## LLM Study Prompts

- Summarise the lecture files for this subject into weekly notes, key concepts, formulas, examples, and assessment relevance.
- Build a glossary for this subject and link each term to shared concepts where possible.
- Create a revision quiz from the learning map and the curated lecture notes.
- Identify which raw files are most useful for assessment preparation and why.

## Maintenance Checklist

- [ ] Weekly notes created
- [ ] Assignment pages created
- [ ] Key notebooks explained
- [ ] Shared concepts linked
- [ ] Exam/revision questions added
