---
type: learning-map
subject: 94693-big-data-engineering
code: 94693
semester: Sem2 2025 Spring
status: active
---

# 94693 Big Data Engineering - Learning Map

## Purpose

Use this page as the curated entry point for the subject. Raw copied files are useful evidence, but this page should become the LLM-friendly map of what matters.

## Core Focus

- Data pipelines and storage
- Distributed processing
- Batch and streaming trade-offs
- Reliability and scalability

## Connected Concepts

- [Big Data](../../03-shared-concepts/big-data.md)
- [Data Science Workflow](../../03-shared-concepts/data-science-workflow.md)
- [Python](../../03-shared-concepts/python.md)

## Study Workflow

1. Review the weekly lecture material in [lectures](lectures/README.md).
2. Convert important source material into concise Markdown notes.
3. Link durable ideas to shared concepts.
4. Track assessment requirements in [assignments](assignments/README.md).
5. Use [questions](questions/README.md) for exam drills and unresolved questions.

## Imported Source Profile

| Bucket | Source Count |
|---|---:|
| Lectures | 16 |
| Assignments | 78 |
| Notebooks | 42 |
| Sources | 3 |

## Key Raw Files To Triage

### Lectures

- Big Data Engineering - Lecture 1.pdf
- Big Data Engineering - Lecture 2.pdf
- Big Data Engineering - Lecture 3.pdf
- Big Data Engineering - Lecture 4.pdf
- Big Data Engineering - Lecture 5.pdf
- Big Data Engineering - Lecture 6.pdf
- Big Data Engineering - Lecture 7.pdf
- Big Data Engineering - Lecture 8.pdf

### Assignments

- BDE_Assignment_1_ManhTuan_Nguyen_HandoverReport.pdf
- part_1.sql
- part_2.sql
- part_3.sql
- part_4.sql
- BDE_Assignment_1_ManhTuan_Nguyen_v1.pdf
- BDE_Assignment_1_ManhTuan_Nguyen_v2.pdf
- BDE_Assignment_1_ManhTuan_Nguyen_v3.pdf
- BDE_Assignment_1_ManhTuan_Nguyen_v4.pdf
- BDE_Assignment_1_ManhTuan_Nguyen_v5.pdf
- bde_at1_cover.pdf
- 94693 Big Data Engineering - Google Docs.pdf
- 94693 Big Data Engineering_AT2_Report.pdf
- 94693 Big Data Engineering_AT2_Report_cover.pdf
- Assignment_2_ManhTuan_Nguyen.ipynb
- BDE_Assignment_2_ManhTuan_Nguyen_HandoverReport.pdf
- Download Dataset.ipynb
- data_dictionary_trip_records_fhv.pdf
- data_dictionary_trip_records_green.pdf
- 1_nyc_taxi_databricks_analytics.ipynb
- 2_model_improvement (1).ipynb
- 2_model_improvement (2).ipynb
- 2_model_improvement (3).ipynb
- 2_model_improvement.ipynb
- nyc_taxi_databricks_analytics (1).ipynb
- nyc_taxi_databricks_analytics (2).ipynb
- trip_record_user_guide.pdf

### Notebooks

- Lab 1 - Exercise 1-solution.ipynb
- Lab 1 - Exercise 2-solution.ipynb
- Lab 2 - Exercise 1-solution.ipynb
- Lab 2 - Exercise 2-solution-old.ipynb
- Lab 2 - Exercise 2-solution.ipynb
- Lab_3_Exercise_1_solution.ipynb
- Lab_3_Exercise_2_solution.ipynb
- Lab_4_Exercise_1_solution.ipynb
- Lab_4_Exercise_2_solution.ipynb
- Lab_5_Exercise_1.ipynb
- Lab_5_Exercise_2.ipynb
- Lab_6_Exercise_1.ipynb
- Lab_6_Exercise_2.ipynb
- Lab_6_Exercise_3.ipynb
- Lab_7_Exercise_1.ipynb
- Lab_7_Exercise_2.ipynb
- Lab_8_Exercise_1.ipynb
- Lab_8_Exercise_2.ipynb

## Assessment Links

- [ASSIGNMENT 1](assignments/assignment-1.md)
- [ASSIGNMENT 2](assignments/assignment-2.md)
- [ASSIGNMENT 3](assignments/assignment-3.md)

## LLM Study Prompts

- Summarise the lecture files into weekly notes, key concepts, and assessment relevance.
- Build a glossary for this subject and link each term to shared concepts where possible.
- Create a revision quiz from the learning map and the curated lecture notes.
- Identify which raw files are most useful for assessment preparation and why.

## Maintenance Checklist

- [x] Weekly notes created (8 sessions)
- [x] Assignment pages created
- [x] Notebook source index created
- [x] Shared concepts linked
- [x] Revision questions added
